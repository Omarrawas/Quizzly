    ...
    if (styles != null) {
      ...
      document.findAllElements('numFmts').forEach((node1) {
        node1.findAllElements('numFmt').forEach((node) {
          ...
          final formatCode = node.getAttribute('formatCode') ?? 'General';
          // numFmtId values < 164 are standard/built-in OOXML formats.
          // Some generators (LibreOffice, Google Sheets, older Excel versions)
          // incorrectly re-declare them inside <numFmts>. This is non-conformant
          // but harmless — we skip them so the file still parses correctly.
          if (numFmtId < 164) return;
